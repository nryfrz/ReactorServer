...
void Log::write(int level, const char *format, ...) {
    ...
    // 在buffer内生成一条对应的日志信息
    {
        ...
        int m = vsnprintf(buff_->BeginWrite(), buff_->writeableSize(), format, vaList);
        va_end(vaList);
        ...
        if(isAsync_ && deque_ && !deque_->full()) { // 异步方式（加入阻塞队列中，等待写线程读取日志信息）
            ...
            fputs(buff_->data(), fp_);   // 同步就直接写入文件
        }
...
void Log::AppendLogLevelTitle_(int level) {
    switch(level) {
    case 0:
        buff_->appendString("[debug]: ", 9);
        break;
    case 1:
        buff_->appendString("[info] : ", 9);
        break;
    case 2:
        buff_->appendString("[warn] : ", 9);
        break;
    case 3:
        buff_->appendString("[error]: ", 9);
        break;
    default:
        buff_->appendString("[info] : ", 9);
        break;
